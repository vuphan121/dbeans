...
- **Cancellable and never stale (2026-09-21):** while a page is loading, the Refresh button becomes ✕ and stops it — the database query is cancelled, not just ignored. Changing the table, filter, sort, or page cancels the request it replaces, and a slow response that arrives late can never overwrite a newer view.

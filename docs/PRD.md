...
- **HTTP request jobs** call an API on schedule. The user chooses GET/POST/PUT/PATCH/DELETE and can provide headers and a request body. Any 2xx response is success; the response body is deliberately discarded rather than imported as data. (This is more precise than “webhook,” which usually means an event-triggered callback.)

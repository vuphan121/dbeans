...
5. Rename: open **Views**, hover a row and click its pencil (or arrow-key to the row and press F2). The dialog opens with the current name selected; type a new name and press Enter. The row updates immediately, a toast confirms, and the view's filters/sort/columns are unchanged. Renaming to a name another view on the same table already uses shows the server's message inline and keeps the dialog open; renaming to the same name closes it without a request; leading and trailing spaces are trimmed.
6. Overwrite: with a filter/sort/column layout applied that differs from a saved view, open **Views**, click that row's save icon (or highlight it and press Ctrl/⌘+S; the browser's own "save page" must not appear). A confirm titled `Overwrite “<name>”?` appears with **Cancel** focused; Cancel changes nothing and sends no request. **Overwrite** replaces the view's filters, sort, rows per page, and column order/visibility with what is applied now (only *applied* filters, not unapplied drafts), leaves its name alone, and shows a toast. Reapplying the view reproduces the new state. If the view was deleted elsewhere, the confirm stays open and shows the server's error.
7. Reload, or sign in from a different browser: the saved views (with their new names) are still there (they live on the server, per user and connection), while the remembered last state from step 1 is per browser. Delete a view with its trash icon; the pencil and trash buttons never trigger each other or apply the view.

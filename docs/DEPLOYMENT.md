...
**Known limitation:** a job's retry delay (`retryDelaySeconds`) sleeps synchronously inside the tick/run request, so a due job with a long delay (or several due jobs retrying in the same tick) risks whatever execution-time ceiling applies to your Vercel plan/config. `RunJobNow` and `JobsTick` get a longer request timeout than the rest of the API (`api.JobExecutionTimeout`, 55s — see `internal/server/server.go`) so this internal limit no longer cuts jobs off well before that budget is used, but it's still a hard ceiling: the configurable per-job HTTP timeout and retry delay (`maxHTTPRequestTimeoutSeconds`, `maxRetryDelaySeconds` in `internal/api/jobs.go`) are deliberately kept well under it, sized for a Hobby-tier deployment. If you're on Pro or higher and want more headroom (Vercel allows `functions.maxDuration` up to 300s there for the classic serverless-function model — unconfirmed whether it applies to this project's Go Framework Preset deployment, so verify before relying on it), raise `JobExecutionTimeout` and the two `jobs.go` constants together; keep retry delays short otherwise, or use the traditional-host path below, which has no such ceiling.
...
**Practical fix: deploy via `git push` to the connected branch, not `vercel deploy` from the subfolder.** A Git-triggered build clones the full repo and applies Root Directory correctly. After a push lands, re-run the alias (see below) against the new deployment. This project's whole deploy workflow is now push → run `scripts/deploy-realias.sh` → both domains follow — `vercel deploy` from inside `backend/` or `frontend/` is no longer part of the loop once Root Directory is set.

**`scripts/deploy-realias.sh` automates the re-alias step (2026-09-17):** run it right after every `git push origin main` (`bash scripts/deploy-realias.sh`, needs the Vercel CLI logged in). It polls `vercel list <project> --json --meta githubCommitSha=<HEAD SHA>` until that exact commit's deployment for both `frontend` and `backend` reports `state: READY`, then runs `vercel alias set` against it — so it always aliases the deployment that actually corresponds to what was just pushed, not just "whatever's newest" (which could race a concurrent deploy). Fails loudly (non-zero exit) on a build error or a timeout (3 minutes) rather than aliasing something stale or wrong. There's no way to make a plain vanity `vercel.app` alias auto-follow deploys the way a real attached Domain does (`vercel domains inspect dbeans.vercel.app` confirms it isn't tracked as one) — so until this project moves to a real custom domain, this script (or the manual command below) is the actual fix, not a workaround.

...
- **Backend runs two ways from the same code** (`backend/internal/server`, shared by both): a plain Go binary (`backend/main.go`, `http.ListenAndServe` — used for local dev and any traditional always-on host) and a Vercel Go serverless function (`backend/api/index.go`, exported `Handler`) that Vercel invokes per-request; `backend/vercel.json` rewrites every path to that one function so the existing chi router still does its own sub-routing unmodified. Both read `DATABASE_URL`, `ALLOWED_ORIGINS`, `CRON_SECRET`, `SEED_USERNAME`/`SEED_PASSWORD` from the environment (see `backend/.env.example`); the binary additionally reads `PORT`. No local disk state either way — everything durable lives in the operator Postgres DB. See [DEPLOYMENT.md](DEPLOYMENT.md) for exact setup steps.
- **Frontend:** a separate Vite static build (`VITE_API_URL` pointing at wherever the backend is deployed), deployed as its own Vercel project (or any static host). The original single-binary plan (`go:embed` of `frontend/dist` into the Go binary) still stands as an alternative packaging for a non-Vercel, single-host deployment — not wired up, since the two-project Vercel split covers the current target.

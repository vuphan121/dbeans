...
6. Reload, or sign in from a different browser: the saved views (with their new names) are still there (they live on the server, per user and connection), while the remembered last state from step 1 is per browser. Delete a view with its trash icon; the pencil and trash buttons never trigger each other or apply the view.

...
- Rows are loaded with server-side pagination (50, 100, or 250 per page), an honest row total (see below), stable primary-key ordering, and server-side single-column sorting. The browser never loads an unbounded table into frontend memory.
...
- **Row totals that scale (2026-09-21):** opening a table never begins with a full `count(*)`. A large, unfiltered table shows the planner's estimate (**~200,000 rows**, pager **1 of ~2,000**); anything else is counted up to 50,000 rows under a 3-second limit, showing an exact number, **50,000+**, or **?** if even that timed out. Whenever the total isn't exact, a **Count exactly** button runs a full count on demand (15-second limit, cancellable). Paging and sorting reuse the total instead of recounting; applying a filter, switching tables, refreshing, and any add/delete recount. Next/Previous never depend on the total — the server always knows whether another page exists.
- **Cancellable and never stale (2026-09-21):** while a page is loading, the Refresh button becomes ✕ and stops it — the database query is terminated, not just ignored, including on the serverless deployment where simply dropping the connection wouldn't stop it. Changing the table, filter, sort, or page cancels the request it replaces, and a slow response that arrives late can never overwrite a newer view.
- **CSV import (2026-09-21):** **Import** on a writable table. Pick a comma-, semicolon-, tab-, or pipe-separated file (quoting and CRLF handled; up to 10,000 rows per import), say whether the first row is a header, and map file columns to table columns (auto-matched by name, any column skippable; generated and identity columns can't be targets and NOT NULL columns without defaults must be mapped). **Treat empty values as NULL** is on by default. **Validate** dry-runs the whole insert on the server in a transaction that is always rolled back and lists every failing row (up to 20) with the database's own message; **Import** then asks for confirmation and writes all rows in one transaction — if any row fails, none are written. Not available on views or read-only connections; recorded in History as a row count (never the row contents). There is deliberately no Undo for an import.
- **Fast navigation and saved views (2026-09-21):** the table selector is searchable (type to filter and rank, arrow keys + Enter), lists **Recent** tables, and lets you **pin** favorites. The Data view remembers, per connection and per table, the last table plus the applied filters, sort, rows per page, column order/visibility, and column widths across reloads — anything that no longer matches the table's columns is quietly dropped. **Views** saves the current filters, sort, page size, and column layout under a name for that table, stored on the server per user and connection so it follows you across browsers; applying one replaces the current view's state. Each row in the Views menu has a pencil to **rename** it (or F2 on the highlighted row), a save icon to **overwrite** it with the filters, sort, rows per page, and column layout currently applied (after a confirm; Ctrl/⌘+S on the highlighted row), and a trash icon to delete it; a name already used for that table is rejected inline.
- **Audit trail and Undo (2026-09-18):** every add, edit, delete (single or bulk), and guarded schema change is logged and readable in the new History tab (see below). A short-lived "Undo" toast follows each add/edit/delete, reversing it by re-running the same insert/update/delete the mutation itself uses — restoring a deleted or edited row's exact prior content, except that a database-generated primary key is reassigned rather than restored to its old value.
...
1. **Import:** *CSV import with column mapping, dry-run validation, and an atomic commit shipped 2026-09-21 (see above).* Still open: imports over 10,000 rows (needs server-side staging to stay atomic), JSON/SQL-dump import, and Undo for an import. Export of the visible page or selected rows is implemented; exporting every row matching a server-side filter still needs a streaming endpoint.
2. **Fast navigation and saved views:** *searchable/pinned tables, remembered last table/filter/sort/column layout per connection, and named saved views shipped 2026-09-21 (see above).* Still open: URL-addressable views, and jumping from a foreign-key value to the referenced row.
3. **Large-table performance:** *cancellable requests, stale-response protection, and estimated/optional exact row counts shipped 2026-09-21 (see above).* Still open: index-aware sort warnings, and keyset paging so very deep pages don't pay for a large `OFFSET`.
4. **Production safety:** per-connection write mode, optional session-level “unlock writes,” stronger visual distinction for production connections, and optimistic-concurrency checks. *(An audit trail of Data-view mutations and single-action Undo shipped 2026-09-18 — see above.)*
...
- Manually save a query (from any editor tab, saved or not) for reuse; rename or delete it later, from the tab itself. Saved queries live on the server, per user (2026-09-21), so they follow you across browsers; queries saved in an older browser are moved over automatically the first time it signs in, then removed from local storage. Edits apply instantly and sync in the background, and reload from the server with a message if a save is rejected.
- **Automatic history (2026-09-18):** a History tab in the workbench (alongside Query/Data/Graphs/ERD) lists every past query run and every Data-editor mutation for that connection, newest first. A query-run entry reopens as a new SQL tab via **Open in query**, without executing it. Backed by the same `analytics_events` table sign-ins were already logged to — see ARCHITECTURE.md §2/§4.
...
- Import beyond a single CSV file per atomic request: JSON/SQL-dump import, and files over 10,000 rows. (A CSV import wizard shipped in the Data view — see Table data browser.)

...
2. **Fast navigation and saved views:** *searchable/pinned tables, remembered last table/filter/sort/column layout per connection, and named saved views shipped 2026-09-21 (see above).* Still open: URL-addressable views, overwriting an existing saved view with the current state (renaming shipped; overwriting is still delete and re-save), and jumping from a foreign-key value to the referenced row.

...
export function DataBrowser({ connection, onOpenQuery, onOpenSql }: { connection: SavedConnection; onOpenQuery: (name: string) => void; onOpenSql: (title: string, sql: string) => void }) {
  ...
  const updateViewConfig = useSavedViewsStore((s) => s.updateViewConfig);
    ...
    try {
      ...
      if (hasPrimaryKey) {
        showToast(removed.length === 1 ? "Row deleted." : `${removed.length} rows deleted.`, {
          label: "Undo",
          onAction: async () => {
            await Promise.all(removed.map((row) => insertTableRow(token, connection.id, { schema: selected.schema, table: selected.table.name, values: nonGeneratedValues(columns, row) })));
            await refresh(true);
          },
        });
      }
